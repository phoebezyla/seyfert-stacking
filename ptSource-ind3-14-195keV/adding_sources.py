...
for ix in IX:
    ...
    for j, e in enumerate(midE):
        ...
        with open("results_csvs/results-%six-%.1fTeV-individual.csv"%(ix,e), newline="") as f:
            ...
            for row in reader:
                ...
                results_df[source_name] = {
                    ...
                    "TS":      float(row["TS"]),
                    "log_val": [float(row[f"log_val_{k}"]) for k in range(nVal)],
                    "norms":   [float(row[f"norms_{k}"]) for k in range(nVal)],
                }
    ...
    #        nullLLH_arr.append(nullLLH)   # no weighing? 
        ...
        try:
            TS_stacked = fj1.compute_TS('stacked',like_df)
            print("Compute TS success!")
        except:
            print("compute_TS failed :(")
        ...
        norms_Stack,log_val_Stack,a_Stack = StackingAnalysis.likelihood_profile(
                 indminNorm,fjl,param_df,like_df,"Stacked",
                 valN=nVal,computeTS=False
        )
    ...
    f = open("%s/results_csvs/normed-long-stacked-ind%s.csv"%(DIR,ix),'w',newline='')
    writer = csv.writer(f)
    writer.writerow(['pivot','indminNorm','TS'] +\
    #,'loglike'] +\
        [f"norms_{i}" for i in range(nVal)] +\
        [f"log_val_{i}" for i in range(nVal)])
