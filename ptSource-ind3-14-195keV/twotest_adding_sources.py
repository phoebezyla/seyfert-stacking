...
## Loop over sources to isolate and remove from stacked profile ##
for ii, cc in enumerate(sourceName):
    ...
    for j, e in enumerate(midE):
        ...
        norms_Stack,log_val_Stack,a_Stack = StackingAnalysis.likelihood_profile(
            indminNorm,fjl,param_df,like_df,"Stacked",computeTS=False)
    ...
    for pivot, vals in results.items():
        all_rows.append([cc,pivot,vals["indminNorm"],vals['TS']])


## Save full results dictionary to one csv per index ##
f = open("%s/results-stacked-ind%s-49sources-exclusion.csv"%(DIR,ix),'w',newline='')
writer = csv.writer(f)
writer.writerow(['excludedSource','pivot','indminNorm','TS'])
for row in all_rows:
    writer.writerow(row)
f.close()
