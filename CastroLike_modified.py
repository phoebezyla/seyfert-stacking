...
class IntervalContainer(object):
    def __init__(
        ...
        self._stop = stop
        self._weight = weight
        self._pivot = pivot if pivot is not None else (start + stop)/2.0
...
class CastroLike(PluginPrototype):
    ...
    @staticmethod
    def _plot(containers, sub, color, model=None):
        ...
        for interval_container in containers:
            ...
            piv_TeV = interval_container.pivot/1e9 # put pivot into TeV
            dt = (interval_container.stop - interval_container.start)/(2.0 * 1e9)
            ...
            if model is not None:
                piv_arr = np.array([interval_container.pivot])
                model_flux = model.get_total_flux(piv_arr)[0] * interval_container.weight
                sub.plot(piv_TeV, model_flux, marker='*', color='r',
                         markersize=10,zorder=5)  

        sub.errorbar(xs, ys, xerr=xerrs, yerr=yerrs, 
                     fmt=",", ecolor=color, mfc=color,label='Data')
        
        sub.errorbar(uls_xs, uls_ys, xerr=uls_xerrs, yerr=uls_yerrs,
                     uplims=True, fmt=",", ecolor=color, mfc=color,
                     label='Upper Limits')
    ...
    def plot(self, plot_model=True, n_points=1000, fig=None, sub=None):
        ...
        model_arg = self._likelihood_model if plot_model else None
        xs, xerrs = self._plot(self._active_containers, sub, "blue", model=model_arg)

        # Find out which containers are not active
        inactive_containers = [c for c in self._interval_containers
                               if c not in self._active_containers]

        if len(inactive_containers) > 0:
            self._plot(inactive_containers, sub, "gray", model=None)

        sub.set_xscale("log")
        sub.set_yscale("log")

        if plot_model and len(xs) > 0:

            xs = np.asarray(xs)
            xerrs = np.asarray(xerrs)

            min_e = (xs.min() - xerrs[xs.argmin()]) * 1e9  # keV
            max_e = (xs.max() + xerrs[xs.argmax()]) * 1e9
            ...
            xx = xx_keV / 1e9
            mean_weight = np.mean([c.weight for c in self._active_containers])
            yy = self._likelihood_model.get_total_flux(xx_keV) * mean_weight

            sub.plot(xx, yy, linestyle="--", color="red", label = 'Model (mean weight)')

        sub.set_xscale('log')
        sub.set_yscale('log')
        sub.set_xlabel('Energy [TeV]',fontsize=12)
        sub.set_ylabel(r'Flux normalization K [keV$^{-1}$ s$^{-1}$ cm$^{-2}$]',fontsize=12)
        sub.set_title('Castrolike data versus model',fontsize=13)
        sub.legend()
